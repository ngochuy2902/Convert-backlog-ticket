function getBranchAndCommit() {
    ...
    const branchName = `git checkout -b ${issueType}/${ticketId}-${filteredChars.map((string) => string.toLowerCase()).join('_')}`.replace(/_+/g, '_');
    const commitMsg = `git add . && git commit -m "[${ticketId}] ${backlogTicketArr.join(' ')}"`;
